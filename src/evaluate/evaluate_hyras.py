...
def evaluate_hyras(years):
    radar_on_hyras = load_output(f'radar_on_hyras_{years[0]}_{years[-1]}.nc').values
    hyras = load_output(f'hyras_{years[0]}_{years[-1]}.nc').values
    # Ensure alignment (just in case)
    mse = np.nanmean((radar_on_hyras - hyras) ** 2)

    print(f"[Evaluate HYRAS] MSE = {mse:.4f}")
    print(f'[Evaluate HYRAS] RMSE = {np.sqrt(mse):.4f}')
    print(f'[Evaluate HYRAS] MAE = {np.nanmean(np.abs(radar_on_hyras - hyras)):.4f}')
    ...
    print(f'[Evaluate HYRAS] Radar-on-HYRAS mean: {np.nanmean(radar_on_hyras):.4f}, max: {np.nanmax(radar_on_hyras):.4f}, min: {np.nanmin(radar_on_hyras):.4f}')

    return mse

if __name__ == "__main__":
    evaluate_hyras(cfg.test_years)
